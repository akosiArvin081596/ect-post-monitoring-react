...
export function Dashboard() {
  ...
  const getStatusBadge = (status: OfflineSurvey['status']) => {
    ...
    return (
      <span className={`rounded-full px-3 py-1 text-base font-medium ${styles[status]}`}>
        {status}
      </span>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5">
        <div className="rounded-2xl border border-blue-100 bg-blue-50/80 p-4 text-base text-blue-800 shadow-sm dark:border-blue-900/40 dark:bg-blue-950/40 dark:text-blue-200">
        ...
        {isLoadingAddresses && (
          <div className="rounded-2xl bg-amber-50 p-4 text-base text-amber-800 shadow-sm dark:bg-amber-900/20 dark:text-amber-200">
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link
                to="/survey/new"
                className="flex-1 rounded-2xl bg-blue-600 px-5 py-4 text-center text-base font-semibold text-white shadow-sm hover:bg-blue-700"
              >
                Start New Survey
              </Link>
              {pendingCount > 0 && isOnline && (
                <button
                  onClick={handleSync}
                  disabled={isSyncing}
                  className="rounded-2xl bg-emerald-600 px-5 py-4 text-base font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50"
                >
